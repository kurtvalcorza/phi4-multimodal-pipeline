...
|---|---|---|---|---|---|
| 2026-09-20 | `1e348e2` / `75ab9bb4` | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-phi4-multimodal` v5) | Default sample path, `Run all` from a fresh interpreter, empty Hugging Face cache, no repository checkout | — | **In flight** — recorded here when the executor returns |
...
**Candidate.** The E2E carrier's default path has executed cleanly on a Kaggle Tesla T4 through every stage that the
tutorial claims — snapshot staging and verification with the remote code hashed, the 4-bit load, the three inference
capabilities, the VizWiz sample, the baselines, the frozen evaluation, the bounded fine-tuning of the vision LoRA, the
held-out evaluation with the asserted improvement, the artifact export and the fresh reload — and failed only on the
exact-caption parity rule of run v4, which `1e348e2` replaces with the held-out-score tolerance. The registry stays at
**Candidate** until the run of blob `75ab9bb4` (in flight) is recorded above as PASSED. The DIMER upload of the weights
is on HOLD by the maintainer's decision (2026-09-20) independently of this gate.
