...
2. **Input boundary:** the reference wrapper caps one request at four images and four audio clips and caps generated output at 2,048 new tokens; larger requests require a separately reviewed serving contract.
3. **Runtime boundary:** the release-reference tutorial requires a CUDA GPU of about 16 GB; the 4-bit load (`quantization="nf4"`) needs CUDA and bitsandbytes, and CPU is not supported for the default path. FlashAttention 2 is not an implicit dependency; callers selecting it must provide a compatible `flash-attn` installation. The default attention implementation is `eager`. For the captioning contract every photograph is downscaled to at most 448 px on its long side before the processor (one or two tiles, about 550 tokens), identically for training and inference.
